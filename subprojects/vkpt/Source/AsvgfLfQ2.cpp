...
namespace
{
// Images used by asvgf_lf.comp (read or written).
constexpr std::array<int, 7> IMAGES_USED = {
    // read (surface data at hi-res and LF history at grad res)
    ...
    // write
    VKPT_IMG_ASVGF_ATROUS_PONG_LF_SH,
    VKPT_IMG_ASVGF_ATROUS_PONG_LF_COCG,
};
}

AsvgfLfQ2::AsvgfLfQ2(VkDevice _device,
...
void AsvgfLfQ2::CreatePipeline()
{
    ...
    // iteration push constant (uint), like Q2RTX pipeline_layout_atrous.
    ...
    VkShaderModule module = shaderManager->GetShaderModule("Q2AsvgfLf");
    if (module == VK_NULL_HANDLE)
    {
        throw RgException(RG_WRONG_ARGUMENT, "Q2AsvgfLf shader module is not loaded");
    }
...
void AsvgfLfQ2::Dispatch(VkCommandBuffer cmd, uint32_t width, uint32_t height)
{
    if (width == 0 || height == 0)
    {
        return;
    ...
    // Q2RTX runs 4 LF iterations, each at GRAD_DWN resolution with the
    // iteration index as push constant.
    const uint32_t wgX = (width / GRAD_DWN + 15) / 16;
    const uint32_t wgY = (height / GRAD_DWN + 15) / 16;

    for (uint32_t i = 0; i < 4; i++)
    {
        vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(i), &i);
