...
namespace
{
// Images used by asvgf_gradient_img.comp (read or written).
constexpr std::array<int, 10> IMAGES_USED = {
    // read
    VKPT_IMG_PT_MOTION,
    VKPT_IMG_PT_COLOR_LF_SH,
    VKPT_IMG_ASVGF_HIST_COLOR_LF_SH_B,
    VKPT_IMG_ASVGF_GRAD_SMPL_POS_A,
    VKPT_IMG_ASVGF_GRAD_HF_SPEC_PING,
    VKPT_IMG_PT_COLOR_HF,
    VKPT_IMG_PT_COLOR_SPEC,
    VKPT_IMG_PT_VIEW_DIRECTION,
    // write
    VKPT_IMG_ASVGF_GRAD_LF_PING,
    VKPT_IMG_ASVGF_GRAD_HF_SPEC_PING,
};
}

AsvgfGradientImgQ2::AsvgfGradientImgQ2(VkDevice _device,
...
AsvgfGradientImgQ2::~AsvgfGradientImgQ2()
{
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}

void AsvgfGradientImgQ2::CreatePipeline()
{
    ...
    VkShaderModule module = shaderManager->GetShaderModule("Q2AsvgfGradientImg");
    if (module == VK_NULL_HANDLE)
    {
        throw RgException(RG_WRONG_ARGUMENT, "Q2AsvgfGradientImg shader module is not loaded");
    }
...
void AsvgfGradientImgQ2::Dispatch(VkCommandBuffer cmd, uint32_t width, uint32_t height)
{
    if (width == 0 || height == 0)
    {
        return;
    ...
    // Q2RTX dispatches asvgf_gradient_img.comp at GRAD_DWN resolution.
